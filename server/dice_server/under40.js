...
//var abi = fs.readFileSync("../dice/build/contracts/DiceUnder40.json", "utf8");
//abi = JSON.parse(abi);

const abiPath = path.resolve(__dirname, '../dice/build/contracts/DiceUnder40.json');
console.log(abiPath);
        ...
        if ((result.number - 1) % 15 === 0) {
            web3.eth.getBlock(result.number - 1).then(result => {
                ...
                ca.methods.getResult(blocknumber).call().then(res => {
                    db.Under40.add(blocknumber, blockhash, res);
                    recycle()
                }).catch(() => {
                    recycle()
                })
