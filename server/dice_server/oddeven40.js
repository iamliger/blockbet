...
//var abi = fs.readFileSync("../dice/build/contracts/DiceOddEven40.json", "utf8"); //mainnet
//abi = JSON.parse(abi);

const abiPath = path.resolve(__dirname, '../dice/build/contracts/DiceOddEven40.json');
console.log(abiPath);
        ...
        if ((result.number - 1) % 15 === 0) {
            web3.eth.getBlock(result.number - 1).then(result => {
                ...
                ca.methods.getResult(blocknumber).call().then(res => {
                    db.OddEven40.add(blocknumber, blockhash, res);
                    recycle()
                }).catch((e) => {
                    console.log(e);
                    recycle()
