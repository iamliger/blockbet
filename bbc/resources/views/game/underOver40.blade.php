	...
	<div class="mainContent">
		...
		<section class="centerArea">
			...
			<!-- Payout on Win -->
				...
				<!-- 게임버튼들 -->				
				<div class="btnArea">
					<button type="button" class="halfBtn blue" onclick="bet(25)">@lang('UNDER BETTING')</button>
					<button type="button" class="halfBtn red" onclick="bet(75)">@lang('OVER BETTING')</button>
				</div>
			</div>
...
@push('scripts')
	...
	<script>
		$(function(){
			$.get(result_url+'/underover40/init',(data)=>{
			...
			function diceresult(){
			$.get('/api/underover40/result?t='+(new Date()).getTime(),(data)=>{
				if(data){
					$('#allBet').empty();
					for(let x in data){
						let bet = data[x];
						let pick = '@lang('UNDER')';
						if(bet['pick'] > 50) pick="@lang('OVER')";
						let color = "";
	...
	function update_payoutToWin(val){
		$('#payout2win').val(numeral(val*pick_rate).format("0,0.0[0000]"));
	}
	...
	function bet(pick){		
		...
		let pickString = (pick === 25 ? 'Under':'Over');
		if(!confirm(`really betting ${amount} at ${pickString}`)) return;
			...
			$.post("/api/betting",{api_token:api_token,game:'underover40',pick:pick,amount:amount,rate:1.95},(data)=>{
	...
	$(function(){
		...
		function diceresult2(){
			$.get(`/api/underover40/myresult?api_token=${api_token}&t=`+(new Date()).getTime(),(data)=>{
				if(data){
					$('#myBet').empty();
					for(let x in data){
						let bet = data[x];
