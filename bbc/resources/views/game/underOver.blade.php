		...
		<section class="centerArea">
			...
			<div id="gauge" class="gaugeInput">
				<div class="row">
					<p class="bg left"></p>
					<p class="bg right"></p>
					<p class="bar ui-slider ui-corner-all ui-slider-horizontal ui-widget ui-widget-content">
...
@push('scripts')
	...
	<script>
		$(function(){
			function diceresult(){
			$.get('/api/underover/result?t='+(new Date()).getTime(),(data)=>{
				if(data){
					$('#allBet').empty();
					for(let x in data){
						let bet = data[x];
	...
	function bet(pick){		
		...
		if(!confirm(`really betting ${amount} at ${pickString}`)) return;
			...
			$.post("/api/betting",{api_token:api_token,game:'underover',pick:pick,amount:amount,rate:1.95},(data)=>{
	...
	$(function(){
		...
		function diceresult2(){
			$.get(`/api/underover/myresult?api_token=${api_token}&t=`+(new Date()).getTime(),(data)=>{
				if(data){
					$('#myBet').empty();
					for(let x in data){
						let bet = data[x];
