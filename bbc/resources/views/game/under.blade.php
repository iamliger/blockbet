		...
		<section class="centerArea">
			...
			<div class="scoreBox">
				...
				<div class="score">
					...
					<p><span class="score2">1.970</span><b>x</b></p>
			...
			<!-- Payout on Win -->
				...
				<!-- 게임버튼들 -->
				<div class="btnArea">
					<button type="button" class="fullBtn" onclick="bet()">@lang('BETTING')</button>
				</div>				
			</div>
...
@push('scripts')
	...
	<script>
		$(function(){
			function diceresult(){
			$.get('/api/under/result?t='+(new Date()).getTime(),(data)=>{
				if(data){
					$('#allBet').empty();
					for(let x in data){
						let bet = data[x];
	...
	function bet(pick){		
		...
		if(!confirm(`really betting ${amount} at ${pick_number} Dice`)) return;
			...
			$.post("/api/betting",{api_token:api_token,game:'under',pick:pick_number,amount:amount,rate:pick_rate},(data)=>{
	...
	$(function(){
		...
		function diceresult2(){
			$.get(`/api/under/myresult?api_token=${api_token}&t=`+(new Date()).getTime(),(data)=>{
				if(data){
					$('#myBet').empty();
					for(let x in data){
						let bet = data[x];
