...
function selectGame(num) {
	...
	// 게임별 설정
		...
		disableGauge('#gauge');
		// 게임버튼 교체
		$('.gameBtn1').css('display', 'flex');
		$('.gameBtn0, .gameBtn2').hide();
		// 스코어박스 색상변경
		$('.scoreBox').addClass('color2').removeClass('color3');
	...
	gaugeInput: function(min, max, init) {
		return this.each(function() {
			...
			bar.on("touchstart touchmove", function(e) {
				...
				bar.slider("value", (max-min)*percent/100 + min);
...
$(document).ready(function() {
	...
	$('#gauge').gaugeInput(0, 100, 50); // 최소, 최대, 초기값
	disableGauge('#gauge');
	// 그리드 바둑판 생성
